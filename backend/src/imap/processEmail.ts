...
export async function processAndStoreEmail(message: ImapMessage, accountId: string): Promise<void> {
    ...
    if (category === "Interested" && process.env.WEBHOOK_URL) {
        // NOTE: You should ensure process.env.WEBHOOK_URL is available
        notifyInterestedEmail({ from, subject, category, body });
    }

    console.log(`Indexed email: ${subject} (${category}) for account: ${accountId}`);
}